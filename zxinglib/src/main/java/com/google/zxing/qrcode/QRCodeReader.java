...
public class QRCodeReader implements Reader {
    ...
    public final Result decode(BinaryBitmap image, Map<DecodeHintType, ?> hints)
        ...
        if (hints != null && hints.containsKey(DecodeHintType.PURE_BARCODE)) {
            ...
            if (mAutoZoom && needZoomCamera(camera, detectorResult.getPoints(), framingRect)) {
                return null;
            }
    ...
    private boolean needZoomCamera(Camera camera, ResultPoint[] points, Rect frameRect) {
        int qrWidth = getQrCodeWidth(points);
        Log.e("QRCodeReader", "qrWidth = " + qrWidth);
        if (qrWidth == 0 || camera == null || frameRect == null) {
            return false;
        }
